...
// aceita "2:06:21" (h:mm:ss), "6:21" (mm:ss) ou só um número de minutos ("90")
function parseTempoEstudoInput(valor) {
  ...
  if (str.includes(":")) {
    ...
    let h = 0, m = 0, s = 0;
    if (partes.length === 3) [h, m, s] = partes;
    else if (partes.length === 2) [m, s] = partes;
    else return null;
...
document.getElementById("btn-tempo-estudo-add").addEventListener("click", () => {
  ...
  const min = parseTempoEstudoInput(document.getElementById("tempo-estudo-min").value);
  if (!min || min <= 0) {
    alert("Informe quanto tempo você estudou — em hh:mm:ss (ex: 2:06:21) ou direto em minutos (ex: 90).");
    return;
  }
  adicionarMinutosEstudo(data, Math.round(min));
...
document.getElementById("tempo-estudo-dias").addEventListener("click", (e) => {
  ...
  if (btn.dataset.action === "editar-tempo") {
    ...
    const m = Math.round(atual % 60);
    const novo = prompt(`Editar tempo estudado em ${formatarDataBR(data)} (hh:mm:ss ou minutos):`, `${h}:${String(m).padStart(2, "0")}:00`);
    if (novo === null) return;
    const min = parseTempoEstudoInput(novo);
    if (min === null || min < 0) { alert("Digite um valor válido, em hh:mm:ss (ex: 2:06:21) ou minutos."); return; }
    const minArred = Math.round(min);
    if (minArred === 0) delete tempoEstudo[data]; else tempoEstudo[data] = minArred;
    saveTempoEstudo(tempoEstudo);
    renderTempoEstudo();
  }
...
function renderQuestaoPratica() {
  ...
  body.innerHTML = `
    ...
    ${q.imagem ? (
      q.imagemTipo === "upload"
        ? `<img class="thumb-rev" style="width:100%; height:auto; max-height:360px; object-fit:contain; margin:6px 0 10px; cursor:zoom-in;" src="${q.imagem}" data-action="ver-imagem" alt="Print da questão">`
        : `<p style="margin:4px 0 10px;"><a href="${escapeHtml(q.imagem)}" target="_blank" rel="noopener">🔗 ver imagem da questão</a></p>`
    ) : ""}
...
document.getElementById("caderno-fonte").addEventListener("change", (e) => {
  aplicarTamanhoFonteSelecao(e.target.value);
  e.target.selectedIndex = 1; // volta pra "Normal" — é uma ferramenta de aplicar, não um estado fixo
});
...
// tamanho de fonte só no trecho selecionado (ex: deixar uma linha maior/menor
// no meio da folha). O execCommand("fontSize") só aceita 1-7, então marcamos
// com o tamanho 7 e trocamos a tag <font> gerada pelo tamanho real em px.
function aplicarTamanhoFonteSelecao(px) {
  const selecao = window.getSelection();
  if (!selecao || selecao.isCollapsed || !elCadernoArea.contains(selecao.anchorNode)) {
    alert("Selecione o trecho (a linha ou palavra) que você quer deixar maior ou menor antes de escolher o tamanho.");
    return;
  }
  elCadernoArea.focus();
  document.execCommand("fontSize", false, "7");
  elCadernoArea.querySelectorAll('font[size="7"]').forEach(f => {
...
/* ==========================================================================
   Modo leitura do caderno — folha por folha, fundo escuro, e exportar em PDF
   ========================================================================== */
const elLeituraOverlay = document.getElementById("leitura-overlay");
const elLeituraTitulo = document.getElementById("leitura-titulo");
const elLeituraConteudo = document.getElementById("leitura-conteudo");
const elLeituraContador = document.getElementById("leitura-contador");
let leituraArray = [];
let leituraIndex = 0;

function renderLeituraAtual() {
  if (leituraArray.length === 0) {
    elLeituraTitulo.textContent = "Nenhuma folha pra mostrar";
    elLeituraConteudo.innerHTML = "";
    elLeituraContador.textContent = "0 / 0";
    return;
  }
  const p = leituraArray[leituraIndex];
  elLeituraTitulo.textContent = p.titulo || "Sem título";
  elLeituraConteudo.innerHTML = p.html || "";
  elLeituraContador.textContent = `${leituraIndex + 1} / ${leituraArray.length}`;
}

function abrirModoLeitura() {
  leituraArray = cadernoListaFiltradaOrdenada();
  if (leituraArray.length === 0) {
    alert("Crie ao menos uma folha no caderno antes de abrir o modo leitura.");
    return;
  }
  const indiceAtual = leituraArray.findIndex(p => p.id === cadernoPaginaAtualId);
  leituraIndex = indiceAtual >= 0 ? indiceAtual : 0;
  renderLeituraAtual();
  elLeituraOverlay.classList.add("aberta");
}
function fecharModoLeitura() {
  elLeituraOverlay.classList.remove("aberta");
}

document.getElementById("btn-caderno-leitura").addEventListener("click", abrirModoLeitura);
document.getElementById("btn-leitura-fechar").addEventListener("click", fecharModoLeitura);
document.getElementById("btn-leitura-anterior").addEventListener("click", () => {
  leituraIndex = Math.max(0, leituraIndex - 1);
  renderLeituraAtual();
});
document.getElementById("btn-leitura-proxima").addEventListener("click", () => {
  leituraIndex = Math.min(leituraArray.length - 1, leituraIndex + 1);
  renderLeituraAtual();
});
document.addEventListener("keydown", (e) => {
  if (!elLeituraOverlay.classList.contains("aberta")) return;
  if (e.key === "Escape") fecharModoLeitura();
  if (e.key === "ArrowLeft") document.getElementById("btn-leitura-anterior").click();
  if (e.key === "ArrowRight") document.getElementById("btn-leitura-proxima").click();
});

document.getElementById("btn-leitura-imprimir").addEventListener("click", () => {
  const area = document.getElementById("caderno-print-area");
  area.innerHTML = leituraArray.map(p => `
    <div class="caderno-print-pagina">
      <h2>${escapeHtml(p.titulo || "Sem título")}</h2>
      <div class="caderno-print-conteudo">${p.html || ""}</div>
    </div>
  `).join("");
  document.body.classList.add("imprimindo-caderno");
  const limpar = () => {
    document.body.classList.remove("imprimindo-caderno");
    area.innerHTML = "";
