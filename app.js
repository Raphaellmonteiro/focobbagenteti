...
document.getElementById("form-registro").addEventListener("submit", (e) => {
  ...
  // limpa a maior parte do formulário, mas mantém matéria / origem / assunto / grupo
  ...
  container.innerHTML = lista.map(q => `
    <div class="rev-card ${q.revisada ? "revisada" : ""}" data-id="${q.id}">
      <div class="rev-top">
        ...
        <span class="rev-meta">${titleCase(q.materia)} · ${q.data}${q.codigo ? " · #" + escapeHtml(q.codigo) : ""}${q.proximaRevisao ? " · revisar em " + formatarDataBR(q.proximaRevisao) : ""}</span>
      </div>
      ...
      <details class="rev-conteudo">
        ...
        <div class="rev-conteudo-body">
          ...
          ${temAlternativas(q.alternativas) ? `<ul>${["A","B","C","D","E"].filter(l => q.alternativas && q.alternativas[l]).map(l => `<li class="${q.gabarito === l ? "gabarito-correto" : ""}">${l}) ${escapeHtml(q.alternativas[l])}</li>`).join("")}</ul>` : ""}
          ${q.gabarito ? `<p><span class="rc-label">Gabarito</span>${q.gabarito}</p>` : ""}
