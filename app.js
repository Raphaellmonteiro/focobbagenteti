...
document.getElementById("btn-exportar").addEventListener("click", () => {
  const backup = { questoes, bizus, tempoEstudo, config };
...
document.getElementById("btn-limpar").addEventListener("click", () => {
  if (questoes.length === 0 && bizus.length === 0 && Object.keys(tempoEstudo).length === 0) {
    alert("Já não há nada registrado.");
    return;
  }
  const confirmar1 = confirm(
    `Isso vai apagar TODAS as ${questoes.length} questões, ${bizus.length} bizus e o tempo de estudo salvos neste navegador.\n\nRecomendado: clique em "Exportar backup" antes, caso queira guardar algo.\n\nContinuar e apagar tudo?`
  );
